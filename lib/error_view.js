...
class ErrorView {
  ...
  toJSON() {
    ...
    const data = this.serializeData(stack);
    return {
      error: data,
    };
  }
  ...
  serializeData(stack, callback) {
    callback = callback || this.serializeFrame.bind(this);
    ...
    return {
      ...
      frames: stack instanceof Array ? stack.filter(frame => frame.getFileName()).map(callback) : [],
    };
  }
